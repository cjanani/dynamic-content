...
function display() {
    ...
    menuConfig.map((menuItem, index)=>{
        ...
        node.onclick = ()=>{
            ...
            if (activeMenu && activeMenu.id === subMenuItem.id) activeMenu.classList.toggle("menuOpen");
        ...
        if (menuItem.subMenu.length > 0) {
            ...
            menuItem.subMenu.map((subTopic, subIndex)=>{
                ...
                listItem.onclick = (event)=>{
                    ...
                    const subMenuListItem = document.getElementById(`menu_${index}_submenuList_${subIndex}`);
